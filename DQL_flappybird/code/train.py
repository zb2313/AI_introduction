...
def train(opt):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=opt.lr)#优化器 Adam更新网络参数 学习率lr
    criterion = nn.MSELoss()#损失函数
    game_state = FlappyBird()
    image, reward, terminal = game_state.next_frame(0)#初始图片 和奖励值
    image = pre_processing(image[:game_state.screen_width, :int(game_state.base_y)], opt.image_size, opt.image_size)#更改图片格式
    image = torch.from_numpy(image)
    if torch.cuda.is_available():
        model.cuda()
        image = image.cuda()
    state = torch.cat(tuple(image for _ in range(4)))[None, :, :, :]#初始状态
    ...
    torch.save(model, "{}/flappy_bird".format(opt.saved_path))#保存模型
